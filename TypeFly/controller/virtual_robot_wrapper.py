...
class VirtualRobotWrapper(RobotWrapper):
    ...
    def start_stream(self):
        print("[V-WRAP] Attempting to open webcam at index 0...")
        self.cap = cv2.VideoCapture(0)
    
        if not self.cap.isOpened():
            print("❌ [V-WRAP] FATAL: COULD NOT OPEN WEBCAM. The video stream will be empty.")
            print("   TROUBLESHOOTING: Is another app using the camera? Do you have permissions?")
            self.stream_on = False
        else:
            print("✅ [V-WRAP] Webcam opened successfully.")
            self.stream_on = True
